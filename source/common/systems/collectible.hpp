  ...
  void update(World *world, const PhysicsSystem *physicsSystem,
    ...
    for (auto entity : world->getEntities()) {
      CollectibleComponent *collectible =
          entity->getComponent<CollectibleComponent>();
      if (!collectible)
        continue;
      ...
      float verticalOffset = glm::sin(collectible->animationTimer *
                                      collectible->bobbingFrequency) *
                             collectible->bobbingHeight;
      entity->localTransform.position.y =
          collectible->initialY + verticalOffset;
      ...
      if (aladdinEntity) {
        ...
        if (physicsSystem) {
          ...
          collected =
              physicsWorld.hasTriggerEvent(aladdinEntity, entity, true) ||
              physicsWorld.hasContactEvent(aladdinEntity, entity, true);
        }
        if (!collected) {
          const glm::vec3 d = aladdinEntity->localTransform.position -
        ...
        if (collected) {
          ...
          case CollectibleComponent::Type::COIN:
            ...
            std::cout << "[CollectibleSystem] Collected Coin! (Value: "
                      << collectible->value
                      << "). Total Coins: " << aladdin->coinCount << std::endl;
            break;
          case CollectibleComponent::Type::GEM:
            aladdin->gemCount += collectible->value;
            std::cout << "[CollectibleSystem] Collected Gem! (Value: "
                      << collectible->value
                      << "). Total Gems: " << aladdin->gemCount << std::endl;
            break;
          case CollectibleComponent::Type::APPLE:
            aladdin->appleCount += collectible->value;
            std::cout << "[CollectibleSystem] Collected Apple! (Value: "
                      << collectible->value
                      << "). Total Apples: " << aladdin->appleCount
                      << std::endl;
            break;
          case CollectibleComponent::Type::KEY:
            aladdin->hasKey = true;
            std::cout << "[CollectibleSystem] Collected Key! Level Exit is now "
                         "active."
                      << std::endl;
            break;
          case CollectibleComponent::Type::HEALTH:
            if (aladdin->health < 100) {
              aladdin->health = glm::min(100, aladdin->health + collectible->value);
              std::cout << "[CollectibleSystem] Collected Health! (Value: "
                        << collectible->value
                        << "). Total Health: " << aladdin->health << std::endl;
            } else {
              actuallyCollected = false;
            }
            break;
          }
          ...
          if (actuallyCollected) {
            ...
            if (collectible->type == CollectibleComponent::Type::HEALTH) {
              AudioSystem::instance().playSound("assets/audio/healthCollected.mp3");
            } else if (collectible->type == CollectibleComponent::Type::APPLE) {
              AudioSystem::instance().playSound("assets/audio/appleCollected.mp3");
            } else {
