...
/**
 * @brief System that handles character movement, jumping, and states for
 * Aladdin.
 *
 * It reads keyboard input to update the entity's position, applies "fake"
 * gravity until a physics system is available, and manages character rotation.
 */
class AladdinControllerSystem {
  ...
  /**
   * Foot ray + contact normals. Concave mesh / triangle contacts often have
   * shallow support normals; capsule can micro-separate while moving, so the
   * ray is always used together with contacts and cast farther than the old
   * 0.25m probe.
   */
  static bool computeGroundedFromPhysics(Entity *entity,
    ...
    if (auto *collider = entity->getComponent<ColliderComponent>()) {
      ...
      case ColliderShape::Box:
        halfHeight = collider->halfExtents.y;
        break;
      case ColliderShape::Sphere:
        halfHeight = collider->radius;
        break;
      case ColliderShape::Capsule:
        halfHeight = (collider->height * 0.5f) + collider->radius;
        break;
      default:
        break;
      }
    ...
    // Probe from slightly above the collider's bottom (feet), accounting for
    // collider center offset.
    const float feetY =
        entity->localTransform.position.y + centerOffsetY - halfHeight;
    const glm::vec3 origin(entity->localTransform.position.x,
                           feetY + kProbeStartAboveFeet,
                           entity->localTransform.position.z);
    const RaycastHit groundHit =
        physicsWorld.raycast(origin, glm::vec3(0.0f, -1.0f, 0.0f), kRayLength);
    const bool fromRay =
        groundHit.hasHit && groundHit.entity && groundHit.entity != entity &&
        groundHit.entity != swordHitbox && groundHit.normal.y >= kMinUpDotRay;
...
public:
  ...
  static bool wasHitInCurrentAttack(AladdinControllerComponent *aladdin,
                                    Entity *target) {
    return std::find(aladdin->hitEntities.begin(), aladdin->hitEntities.end(),
                     target) != aladdin->hitEntities.end();
  }

  static void markHitInCurrentAttack(AladdinControllerComponent *aladdin,
                                     Entity *target) {
  ...
  static void updateSwordHitboxTransform(Entity *player, Entity *swordHitbox) {
    ...
    const glm::vec3 forward =
        glm::normalize(glm::vec3(glm::sin(yaw), 0.0f, glm::cos(yaw)));
    const glm::vec3 offset = forward * 1.1f + glm::vec3(0.0f, 0.0f, 0.0f);

    swordHitbox->localTransform.position =
        player->localTransform.position + offset;
    swordHitbox->localTransform.rotation = glm::vec3(0.0f, yaw, 0.0f);
    swordHitbox->localTransform.scale = glm::vec3(1.0f);
  }

  /**
   * @brief Updates every entity in the world containing a
  ...
  void update(World *world, PhysicsSystem *physicsSystem, float deltaTime) {
    ...
    for (auto entity : players) {
      AladdinControllerComponent *aladdin =
          entity->getComponent<AladdinControllerComponent>();
      if (!aladdin)
        continue;
      ...
      if (app->getConfig().contains("game")) {
        aladdin->mouseSensitivity = app->getConfig()["game"].value(
            "mouseSensitivity", aladdin->mouseSensitivity);
      }
      ...
      if (aladdin->isAiming) {
        ...
        aladdin->cameraOrbitPitch = glm::clamp(aladdin->cameraOrbitPitch,
      ...
      // Camera forward/right on XZ plane (from orbit yaw, NOT from character
      // yaw)
      const float orbYaw = aladdin->cameraOrbitYaw;
      const glm::vec3 camFwd =
          glm::vec3(-glm::sin(orbYaw), 0.0f, -glm::cos(orbYaw));
      const glm::vec3 camRight =
          glm::vec3(glm::cos(orbYaw), 0.0f, -glm::sin(orbYaw));

      glm::vec3 moveDir = camFwd * inputFwd + camRight * inputRight;
      const bool hasMoveInput = glm::length(moveDir) > 0.001f;
      if (hasMoveInput) {
        moveDir = glm::normalize(moveDir);
        // Smoothly rotate character to face the movement direction (exponential
        // smoothing)
        ...
        const float smoothFactor =
            1.0f - glm::exp(-aladdin->rotationSpeed * deltaTime);
        aladdin->facingYaw += diff * smoothFactor;
        while (aladdin->facingYaw > glm::pi<float>())
          aladdin->facingYaw -= 2.0f * glm::pi<float>();
        while (aladdin->facingYaw < -glm::pi<float>())
          aladdin->facingYaw += 2.0f * glm::pi<float>();
      }
      ...
      // 3. Handle gravity, movement and jumping via real physics (if body
      // exists)
      if (hasPhysicsBody) {
        auto &physicsWorld = physicsSystem->getPhysicsWorld();
        Entity *sword =
            (swordHitboxes.count(entity) > 0) ? swordHitboxes[entity] : nullptr;
        // Pre-step grounding (jump): same tolerances; postPhysics will refresh
        // after step for animation.
        aladdin->isGrounded =
            computeGroundedFromPhysics(entity, physicsWorld, sword);
        ...
        if (keyboard.justPressed(GLFW_KEY_SPACE) && aladdin->isGrounded &&
            !aladdin->isJumpPreparing) {
          aladdin->isJumpPreparing = true;
          aladdin->jumpDelayTimer =
              0.32f; // Increased delay to match animation takeoff better
        }
        ...
        // We will update aladdin->velocity in postPhysicsUpdate from the actual
        // RB velocity
      } else {
        // Fallback for scenes that still do not have a rigid body setup yet
        if (hasMoveInput) {
          entity->localTransform.position +=
              moveDir * aladdin->speed * deltaTime;
        }
      }
      ...
      if (keyboard.justPressed(GLFW_KEY_F) && !aladdin->isAttacking) {
        ...
        aladdin->hitEntities
            .clear(); // Clear the list of entities hit in the previous attack
      ...
      if (keyboard.isPressed(GLFW_KEY_R) && aladdin->appleCount > 0) {
        if (!aladdin->isAiming) {
          ...
          glm::vec2 windowSize = app->getWindowSize();
          glfwSetCursorPos(app->getWindow(), windowSize.x * 0.5,
                           windowSize.y * 0.5);
        ...
        if (cameraEntity) {
          ...
          glm::vec3 camFwd = glm::normalize(
              glm::vec3(-glm::cos(cpitch) * glm::sin(cyaw), glm::sin(cpitch),
                        -glm::cos(cpitch) * glm::cos(cyaw)));
          glm::vec3 camRight =
              glm::normalize(glm::cross(camFwd, glm::vec3(0, 1, 0)));
          glm::vec3 camUp = glm::normalize(glm::cross(camRight, camFwd));

          // Apply aim offset (normalized by screen sensitivity)
          float sens = 0.002f;
          throwDir =
              glm::normalize(camFwd + (aladdin->aimOffset.x * sens * camRight) -
                             (aladdin->aimOffset.y * sens * camUp));
        } else {
          const float pyaw = aladdin->facingYaw;
          throwDir =
              glm::normalize(glm::vec3(-glm::sin(pyaw), 0.2f, -glm::cos(pyaw)));
        }

        // Position at Aladdin's hands
        const float pyaw = aladdin->facingYaw;
        const glm::vec3 playerFwd =
            glm::vec3(-glm::sin(pyaw), 0.0f, -glm::cos(pyaw));
        apple->localTransform.position = entity->localTransform.position +
                                         glm::vec3(0, 1.3f, 0) +
                                         playerFwd * 0.8f;
        ...
        if (throwVel.y < 3.0f)
          throwVel.y += 3.0f; // Slight upward arc
  ...
  /**
   * Run after PhysicsSystem::update so overlap events and player position match
   * the stepped world. Handles sword hits and third-person camera follow.
   */
  void postPhysicsUpdate(World *world, PhysicsSystem *physicsSystem,
                         float deltaTime) {
    if (!world)
      return;
    ...
    for (auto entity : world->getEntities()) {
      ...
      if (aladdin->isAttacking) {
        for (auto other : world->getEntities()) {
          if (other == entity || other == swordHitbox)
            continue;
          ...
          if (physicsSystem) {
            auto &physicsWorld = physicsSystem->getPhysicsWorld();
            swordOverlap =
                physicsWorld.hasAnyInteraction(swordHitbox, other, true);
          }
          if (!swordOverlap)
            continue;
          ...
          if (enemy && enemy->currentState != EnemyComponent::State::DEAD) {
            if (wasHitInCurrentAttack(aladdin, other))
              continue;
            enemy->health -= 25;
          ...
          BreakableComponent *breakable =
              other->getComponent<BreakableComponent>();
          if (breakable) {
            if (wasHitInCurrentAttack(aladdin, other))
              continue;
            markHitInCurrentAttack(aladdin, other);
            ...
            for (size_t i = 0; i < breakable->lootItems.size(); ++i) {
              float angle = ((float)i / (float)breakable->lootItems.size()) *
                            2.0f * glm::pi<float>();
              float radius = 4.0f; // Wider scatter to prevent instant pickup
              glm::vec3 scatterOffset = glm::vec3(
                  glm::cos(angle) * radius, 1.2f, glm::sin(angle) * radius);
              requests.push_back(
                  {other->localTransform.position + scatterOffset,
                   breakable->lootItems[i]});
            }
            ...
            for (const auto &req : requests) {
              ...
              coll->pickupDelay =
                  0.6f; // Delay pickup so they can be seen spawning
      ...
      // ── 5. Sync State & Animation (Moved here to use actual physics
      // velocity) ── Debounced grounded:
      ...
      if (rbComp && rbComp->bodyHandle) {
        aladdin->velocity =
            physicsSystem->getPhysicsWorld().getLinearVelocity(entity);
        auto &physicsWorld = physicsSystem->getPhysicsWorld();
        const bool groundedRaw =
            computeGroundedFromPhysics(entity, physicsWorld, swordHitbox);

        const float vy = aladdin->velocity.y;
        constexpr float kCoyoteSeconds =
            0.30f; // longer to absorb concave-mesh flicker
        constexpr float kFallVyKillCoyote =
            -6.0f; // only a real fall kills coyote
        constexpr float kCoyoteMaxRiseVy = 2.8f;
        constexpr float kFallVyKillMinAirborne =
            0.06f; // ignore single-frame vy spikes
        ...
        // Only consider grounded for animation/logic if NOT moving upwards
        // significantly
        const bool isMovingUp = aladdin->velocity.y > 0.5f;

        groundedSmooth = (groundedRaw && !isMovingUp) ||
                         (aladdin->groundedCoyoteTimer > 0.0f &&
                          vy > kFallVyKillCoyote && vy < kCoyoteMaxRiseVy);
        ...
        airborneForAnim =
            ...
            (aladdin->airborneTimer > aladdin->airborneAnimDelay ||
             vy < aladdin->fallAnimMinDownSpeed || vy > kRisingFromJumpVy);
      }
      ...
      if (animPtr) {
        ...
        float horizontalSpeed =
            glm::length(glm::vec2(aladdin->velocity.x, aladdin->velocity.z));
        ...
        if (aladdin->isAttacking) {
          ...
          if (aladdin->isJumpPreparing) {
            ...
            if (physicsSystem) {
              ...
              auto hit = pw.raycast(entity->localTransform.position,
                                    {0.0f, -1.0f, 0.0f}, 20.0f);
              if (hit.hasHit && hit.entity != entity)
            ...
            if (aladdin->velocity.y > 0.5f) {
              ...
              // If we are high up (> 3m), crawl the animation (0.1x) to stay in
              // the air pose. As we get very close to the ground (< 0.5m),
              // speed up significantly (2.5x) to snap into the landing frames.
              playbackSpeed = glm::mix(
                  2.5f, 0.1f, glm::clamp(heightAboveGround / 3.0f, 0.0f, 1.0f));
            }
          }
        ...
        if (animPtr->currentClipName() != targetClip ||
            std::abs(currentSpeed - playbackSpeed) > 0.05f) {
   ...
   */
  void updateFollowCamera(World *world, PhysicsSystem *physicsSystem,
                          float deltaTime) {
    if (!world || !app)
      return;
    ...
    for (auto entity : world->getEntities()) {
      AladdinControllerComponent *aladdin =
          entity->getComponent<AladdinControllerComponent>();
      if (!aladdin || aladdin->lives <= 0 || !aladdin->enableCameraFollow)
        continue;
      ...
      const glm::vec3 focusPoint =
          playerPos + glm::vec3(0.0f, aladdin->cameraFocusHeight, 0.0f);
      ...
      if (aladdin->cameraMode == AladdinCameraMode::ThirdPerson) {
        ...
        const glm::vec3 armDir = glm::normalize(glm::vec3(
            glm::cos(orbPitch) * glm::sin(orbYaw), -glm::sin(orbPitch),
        ...
        if (physicsSystem) {
          ...
          if (hit.hasHit && hit.entity && hit.entity != entity) {
            float wallDist = glm::max(hit.distance - aladdin->cameraWallOffset,
        ...
        if (desiredDist < aladdin->currentArmDist) {
          ...
          aladdin->currentArmDist +=
              (desiredDist - aladdin->currentArmDist) *
              glm::min(1.0f, aladdin->cameraArmRecoverSpeed * deltaTime);
        }

        // ── Final camera position ──
        const glm::vec3 targetPosition =
            focusPoint + armDir * aladdin->currentArmDist;
        ...
        if (aladdin->cameraSmoothing > 0.0f) {
          ...
          const float factor =
              1.0f - glm::exp(-aladdin->cameraSmoothing * camDt);
          ...
          while (dyaw > glm::pi<float>())
            dyaw -= 2.0f * glm::pi<float>();
          while (dyaw < -glm::pi<float>())
            dyaw += 2.0f * glm::pi<float>();
          r.y += dyaw * factor;
          r.z = 0.0f;
        ...
        const glm::vec3 fpForward =
            glm::normalize(glm::vec3(-glm::sin(fpYaw), 0.0f, -glm::cos(fpYaw)));
        const glm::vec3 fpRight =
            glm::normalize(glm::cross(fpForward, glm::vec3(0.0f, 1.0f, 0.0f)));
        const glm::vec3 fpUp(0.0f, 1.0f, 0.0f);
        const glm::vec3 targetPosition =
            playerPos + fpRight * aladdin->firstPersonCameraOffset.x +
            fpUp * aladdin->firstPersonCameraOffset.y +
            fpForward * aladdin->firstPersonCameraOffset.z;
        const glm::vec3 targetRotation = {aladdin->firstPersonPitch, fpYaw,
                                          0.0f};
      ...
      // Toggle visibility for first-person camera: prefer new component, fall
      // back to legacy
      if (auto *smr = entity->getComponent<SkinnedMeshRendererComponent>()) {
        smr->visible = (aladdin->cameraMode != AladdinCameraMode::FirstPerson);
      } else if (auto *meshRenderer =
                     entity->getComponent<MeshRendererComponent>()) {
        meshRenderer->visible =
            (aladdin->cameraMode != AladdinCameraMode::FirstPerson);
      }
    }
  }
  ...
  void onImmediateGui(World *world) {
    ...
    ImGui::Begin("Aladdin HUD", nullptr,
    ...
    ImVec4 healthColor =
        ImVec4(1.0f - healthFraction, healthFraction, 0.0f, 1.0f);
    ...
    ImGui::Text("Coins: %d | Gems: %d | Apples: %d", aladdin->coinCount,
                aladdin->gemCount, aladdin->appleCount);
    ...
    if (aladdin->invincibilityTimer > 0.0f) {
      ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), "INVINCIBLE: %.1fs",
                         aladdin->invincibilityTimer);
    }

    ImGui::Separator();
    ImGui::Text("Respawn Position:");
    ImGui::Text("%.1f, %.1f, %.1f", aladdin->respawnPosition.x,
                aladdin->respawnPosition.y, aladdin->respawnPosition.z);
    ...
    if (aladdin->isAttacking) {
      ImGui::ProgressBar(aladdin->attackTimer / 0.4f, ImVec2(0.0f, 0.0f),
                         "Sword Attack");
    } else {
    ...
    if (aladdin->isThrowing) {
      ImGui::ProgressBar(aladdin->throwTimer / 0.3f, ImVec2(0.0f, 0.0f),
                         "Throwing Apple");
    } else {
    ...
    ImGui::DragFloat("Rotation Speed", &aladdin->rotationSpeed, 0.1f, 0.0f,
                     20.0f);
    ImGui::DragFloat("Airborne Anim Delay", &aladdin->airborneAnimDelay, 0.005f,
                     0.0f, 0.5f);
    ImGui::DragFloat("Fall Anim Min Down Speed", &aladdin->fallAnimMinDownSpeed,
                     0.05f, -20.0f, 0.0f);
    ...
    int camMode =
        (aladdin->cameraMode == AladdinCameraMode::FirstPerson) ? 1 : 0;
    if (ImGui::Combo("Camera mode", &camMode, "Third person\0First person\0")) {
      aladdin->cameraMode = camMode ? AladdinCameraMode::FirstPerson
                                    : AladdinCameraMode::ThirdPerson;
    }
    ImGui::DragFloat3("First-person offset",
                      &aladdin->firstPersonCameraOffset[0], 0.02f);
    ImGui::DragFloat("First-person pitch (rad)", &aladdin->firstPersonPitch,
                     0.01f, -1.2f, 1.2f);
    ImGui::DragFloat("Camera Smoothing (0=snap)", &aladdin->cameraSmoothing,
                     0.1f, 0.0f, 20.0f);
    ImGui::Text("Orbit Camera (Right-click drag):");
    ImGui::DragFloat("Mouse Sensitivity", &aladdin->mouseSensitivity, 0.0005f,
                     0.001f, 0.02f);
    ImGui::Value("Orbit Yaw", aladdin->cameraOrbitYaw);
    ImGui::Value("Orbit Pitch", aladdin->cameraOrbitPitch);
    ImGui::Text("Spring Arm:");
    ImGui::DragFloat("Arm Length", &aladdin->cameraArmLength, 0.1f, 1.0f,
                     20.0f);
    ImGui::DragFloat("Arm Min Dist", &aladdin->cameraArmMinDist, 0.05f, 0.2f,
                     5.0f);
    ImGui::DragFloat("Wall Offset", &aladdin->cameraWallOffset, 0.05f, 0.0f,
                     2.0f);
    ImGui::DragFloat("Recover Speed", &aladdin->cameraArmRecoverSpeed, 0.1f,
                     0.5f, 20.0f);
    ImGui::DragFloat("Focus Height", &aladdin->cameraFocusHeight, 0.05f, 0.0f,
                     5.0f);
